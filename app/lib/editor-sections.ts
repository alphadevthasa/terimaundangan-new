...
export const editorSections: EditorSection[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: 'story-slide-effect', label: 'Efek Slide', type: 'select', defaultValue: 'timeline',
        options: [
...
// ==================== Per-Template Section Filters ====================
// Maps template name → which sections (and which fields within each) to show
// `null` = include all default fields in that section

type SectionFilter = Record<string, string[] | null>;

const TEMPLATE_SECTION_FILTER: Record<string, SectionFilter | null> = {
  // Elite Wedding: all sections, all fields
  'Elite Wedding': null,

  // Honey Wedding: no verse/groom-role/groom-dad/groom-mom/bride-role/bride-dad/bride-mom/couple/story/rsvp/stream/wishes/closing
  'Honey Wedding': {
  ...
  // Forest Nature: no date-text/couple/groom-full/groom-role/groom-dad/groom-mom/bride-full/bride-role/bride-dad/bride-mom/story/rsvp/stream/wishes/closing-fam/gal-5/gal-6
  'Forest Nature': {
    cover: ['bride-nick', 'groom-nick'], // no date-text
    countdown: null,
    verse: null,
    groom: ['groom-photo'],
    bride: ['bride-photo'],
  ...
  // Java Batik: no couple/groom-role/groom-dad/groom-mom/bride-role/bride-dad/bride-mom/verse/story/rsvp/stream/wishes
  'Java Batik': {
    cover: null,
    countdown: null,
    groom: ['groom-full', 'groom-photo'],
  ...
  // West Sumatra: no keyMap, uses same convention as Elite (e- prefix → kebab keys)
  // Keep all sections similar to default
  'West Sumatra': null,
};
...
export function getTemplateSections(templateName: string): EditorSection[] {
  ...
  if (!filter) return editorSections; // default = all sections

  const result: EditorSection[] = [];

  for (const section of editorSections) {
    const fieldFilter = filter[section.id];
    if (fieldFilter === undefined) continue; // skip this section entirely

    result.push({
      ...section,
      fields: fieldFilter === null
        ? section.fields // include all fields
